...
//$this->db->order_by();
			...
			$g_jab=$this->m->grups_get($id_jab1);
			...
			$g_jabs2=array_merge($jabs2,$id_jab1);
			}
			//print_r($g_jabs2);die();
			//if(!empty($jabatan3)){
...
//cek dulu kalau sama dia gak boleh save
							...
							if(empty($res)){
								...
								if(!empty($pegid)){
									...
									$this->db->where('id_kpi',$pegid[0]->id);
									$kid = $this->db->get('his_kpi_detail')->num_rows();
									//print_r($kid);die();
									if($kid!=0){
										$peg= $peg1->id;
										$data=array(
...
$this->db->where('m_penilaian_kpi.child',$id_parent);
$this->db->where('m_penilaian_kpi.grup',$group_group);
$cek = $this->db->get('m_penilaian_kpi');
//print_r($cek->num_rows());die();
$da=$cek->row_array();
//print_r($da);die();
if($cek->num_rows()==1){
if($da['id_grup']==$id_group){
