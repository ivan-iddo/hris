...
public function getgroup_get(){
	...
	if (array_key_exists('Authorization', $headers) && !empty($headers['Authorization'])) {
		...
		if ($decodedToken != false) {
			...
			if(!empty($id)){
				$this->db->where($this->tabel2.'.child',$this->input->get('id'));
			}

			$this->db->select($this->tabel2.'.*,'.$this->tabel.'.nama as nama_grup');
			$this->db->where($this->tabel2.'.tampilkan','1');
			$this->db->join($this->tabel,$this->tabel.'.id = '.$this->tabel2.'.child');
...
public function delete_get(){
	...
	if (array_key_exists('Authorization', $headers) && !empty($headers['Authorization'])) {
		...
		if ($decodedToken != false) {
			...
			$this->db->where('id',$id);
			$this->db->update($this->tabel,array('tampilkan'=>'0'));
...
public function delete_detail_get(){
	...
	if (array_key_exists('Authorization', $headers) && !empty($headers['Authorization'])) {
		...
		if ($decodedToken != false) {
			...
			$id    = $this->input->get('id_group');
			$this->db->where('id',$id);
			$this->db->update($this->tabel2,array('tampilkan'=>'0'));
