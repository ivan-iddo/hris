        ...
        if (array_key_exists('Authorization', $headers) && !empty($headers['Authorization'])) {
            ...
            if ($decodedToken != false) {
				...
				if(!empty($this->input->get('id'))){
					$this->db->where('migrasi_group_jabatan_ASN_id',$this->input->get('id'));
				}
				...
				$pagination = create_pagination_endless('/m/Group_jabatan_asn/0/', $total_rows,$this->perpage,5);
		...
		$headers = $this->input->request_headers();
			...
			if (array_key_exists('Authorization', $headers) && !empty($headers['Authorization'])) {
				...
				if ($decodedToken != false) {
				 ...
				 if(!empty($this->uri->segment('4'))){
					$this->db->where('migrasi_group_jabatan_ASN_id',$this->uri->segment('4'));
				 }
					 $this->db->order_by('kd_grp_job_profesi','ASC');
			  ...
			  foreach($res as $d){
				$arr['result'][]=array('label'=>$d->ds_group_jabatan,'value'=>$d->migrasi_group_jabatan_ASN_id);
			  }
			}else{
